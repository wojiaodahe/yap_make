...
/* 
 *  对以下使用 struct packed_ptr_int 这个 SB 结构体的说明：
 *  ARM V4 架构不支持地址非对齐访问
...
void tcp_build_options(struct sk_buff *skb, int if_mss, int mss, int if_ts, int if_sack, \
                          int if_wscale, int wacale, unsigned int tstamp, unsigned int ts_recent)
{
    struct packed_ptr_int *pptr;
    struct tcphdr *tcph;
    int doff = SIZEOF_TCPHDR;

    tcph = get_tcph(skb);
    pptr = (struct packed_ptr_int *)(tcph + 1);
    
    if (if_mss)
    {
        pptr->ptr = htonl((TCPOPT_MSS << 24) | (TCPOLEN_MSS << 16) | mss);
        pptr++;
    ...
    if (if_ts)
    {
        if (if_sack)
        {
            pptr->ptr = htonl((TCPOPT_SACK_PERM  << 24) |\
        ...
        {
			pptr->ptr = htonl((TCPOPT_NOP << 24) |
				       (TCPOPT_NOP << 16) |
				       (TCPOPT_TIMESTAMP << 8) |
				       TCPOLEN_TIMESTAMP);
            pptr++;
        }
		pptr->ptr = htonl(tstamp);		
        pptr++;
		pptr->ptr = htonl(ts_recent);	
        pptr++;
